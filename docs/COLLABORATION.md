...
Owner 接受 implementation workstream 后、开始实现前，必须把接受记录和冻结合同固定在
一个可引用的 Git commit（`acceptance anchor`）。Active Agent 的分支或 worktree 必须从
该 anchor 或其后代建立，并在 handoff 中记录 anchor 与实际 base commit。

- Agent 不得从仅含 Proposed 合同或未提交治理修改的工作树开始实现。
- 如果接受记录、合同修订或 Owner 指定的 amendment 尚未进入可引用 commit，Agent 只能
  继续讨论或准备，不得开始 production implementation。
- Accepted 后的合同变化仍只允许通过 `Amendments` 追加；不得用候选实现反向改写冻结段落。
...
candidate evidence 也是追加式记录：失败、被拒绝或被后续 candidate 替代的目录仍保留。
清理产物不得删除已归档 candidate evidence。harness 必须要求明确的 candidate 标识并
只写入该 candidate 的目录；它不得默认写入 research baseline、历史矩阵或共享的“最新”
结果路径。
...
- candidate-specific harness 必须断言合同的完整引擎矩阵：要求 `Supported` 的路径不得以
  `NO-BUILD`、`Fallback` 或遗漏通过；允许 `Not implemented` 的路径必须逐项列明；
- harness 的默认输出路径不得是历史 evidence；候选标识、输出目录和输入 fixture 集必须在
  运行开始时记录，并在缺失或不匹配时 fail-closed；
...
3. Non-trivial work 先提交 Proposed contract；Owner 接受后先固定 acceptance anchor，才能实现。
4. Agent 从 acceptance anchor 建立独立分支或 worktree，先记录 evidence-isolation
   preflight，再按合同实现且不扩大 scope。

...
能解释执行真实 `javac --release 8` 字节码的运行时内核：多线程、监视器、
异常、集合、字符串、网络 socket 程序均可运行且与真机 JVM 输出对照一致；
字节码验证器（结构层+数据流类型检查）默认开启；**AOT 未开始**，
反射 API 与宽类库面是当前最大缺口。
...
| 未知引用类对的验证放行 | DEV-0001 残留 | 极端混淆输入 |
...
volatile 标志忽略（DEV-0007）、socket 读未接中断唤醒（DEV-0008）、
守护线程规则未实现（DEV-0006）、SOE 数值模型不同（DEV-0004）、
未知引用类对验证放行（DEV-0001 残留）。已关闭：监视器重入(0003)、SOE 缺失(0004)、数据流验证缺失(0001 主项)。

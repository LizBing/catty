...
| DEV-0010 | 反射 v1 边界：①访问控制不生效（private 可 get/set，setAccessible 无需调用）②仅 getDeclared*（不含继承成员）③无泛型签名/注解元数据 | 反射最小面（P-0011）按框架核心需求裁剪：枚举/读写/invoke/newInstance 已覆盖 Jackson 式用法 | 范围裁剪而非机制缺陷；访问控制可后补 | 使用反射的第三方库 | 后续轮按真实库需求扩展 |

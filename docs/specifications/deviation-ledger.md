...
| DEV-0010 | 反射边界（v2 后）：①访问控制不生效（private 可 get/set，setAccessible 无需调用）②getMethods 不含接口方法（含 default）③无泛型签名（ParameterizedType 未实现）；注解元数据已保留并可读取 ④getDeclaredMethods/getMethods 按名排序（JVM 未规范顺序） | 反射最小面（P-0011）按框架核心需求裁剪：枚举/读写/invoke/newInstance 已覆盖 Jackson 式用法 | 范围裁剪而非机制缺陷；访问控制可后补 | 使用反射的第三方库 | 后续轮按真实库需求扩展 |

...
| DEV-0008 | R4 阻塞操作可中断（socket 读） | Socket InputStream.read 阻塞不可被 Thread.interrupt 唤醒（未接 SetDeadline 唤醒通路） | M2 最小网络面范围裁剪 | 中断期间挂死读 | DEBT-0011 || DEV-0009 | JDK ArrayList.get 越界异常消息格式 | Catty: "Index: N, Size: M"; HotSpot: "Index N out of bounds for length M" | 类库实现细节，非 JVMS 规范要求 | 仅影响异常消息文本，不影响类型/流控 | 接受并记录 |
